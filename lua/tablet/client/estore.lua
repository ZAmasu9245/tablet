...
function APP:onOpen(layout, frame)
	...
	for _, cat in pairs(cats) do
		for _, itm in pairs(cat.members) do
			if tablet.config.hideNonBuyable and !canBuy(itm) then continue end
	...
	for k, v in SortedPairsByMemberValue(cats, "sortOrder") do
		...
		b.Paint = function(self, w, h)
			...
			local tw, th = ui.DrawText(num == 1 and "All" or v.name, "tablet_estorebtn", w / 2, h / 2, (self.Hovered and !sel) and Color(6, 84, 186) or Color(80, 80, 80), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	local scroll = vgui.Create("mgScrollPanel", layout)
	scroll:SetPos(5, 40)
	...
	List.Load = function(self, id)
		...
		for k, v in SortedPairsByMemberValue(itms, "sortOrder") do
			...
			p.Paint = function(self, w, h)
				...
				ui.DrawText(v.price .. "$", "tablet_estorebtn", h + 5, 5 + th, canAfford and Color(120, 120, 120) or mgui.Colors.Red)
			...
			buy.Paint = function(self, w, h)
				...
				ui.DrawText("Buy", "DermaDefaultBold", w / 2, h / 2, bCan and Color(120, 120, 120) or Color(244, 67, 54, 230), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

				self.x = p:GetWide() - w - 5

				if reason then
					surface.DisableClipping(true)
						ui.DrawText(reason, "tablet_estorebtn", -5, h / 2 - 1, mgui.Colors.Red, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	...
	if firstB then 
		...
		b:SetText("")
		b.Paint = function(self, w, h)
			ui.DrawText("Store is empty.", "tablet_estorebtn", w / 2, h / 2, Color(80, 80, 80), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end
end
