...
function APP:onOpen(layout, frame)
	...
	header.Paint = function(self, w, h)
		...
		ui.DrawText("GUN SHOP", "tablet_gunstoreheader", w / 2, 32, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
	...
	filter.search = vgui.Create("mgTextEntry", filter)
	filter.search:SetSize(filter:GetWide() - 10, 25)
	filter.search:SetPos(5, 5)
	filter.search:SetGhostText("Search...")
	filter.search.OnEnter = function() items:Load() end

	filter.scroll = vgui.Create("mgScrollPanel", filter)
	...
	for catName, _ in pairs(categoriesList) do
		...
		p.Paint = function(self, w, h)
			...
			ui.DrawText(catName, "mgui_button_text", 26, h / 2, self:GetColor(), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end
	...
	local itemsScroll = vgui.Create("mgScrollPanel", layout)
	...
	items.Load = function(self)
		...
		for k, v in SortedPairsByMemberValue(itms, "sortOrder") do
			if tablet.config.hideNonBuyable and !canBuy(v) then continue end
			...
			p.PaintOver = function(self, w, h)
				...
				local _, th = ui.DrawText(v.name, "mgui_button_text", h + 10, 5, Color(255, 255, 255, 50))
				ui.DrawText(price .. "$ " .. (isShipment and " (" .. tostring(v.amount) .. " items)" or ""), "mgui_button_text", h + 10, 5 + th + 5, money >= price and Color(255, 255, 255, 50) or mgui.Colors.Red)
				...
				if reason then
					ui.DrawText(reason, "DermaDefaultBold", w - 10, h / 2, mgui.Colors.Red, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
				end
			end
			...
			buy:SetText("Buy" .. (isShipment and " a Shipment" or ""))
			p.buy = buy
