...
function APP:onOpen(layout, frame)
	...
	header.Paint = function(self, w, h)
		...
		ui.DrawText("Work.GM", "workgm_title", h * 0.15, h / 2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
	...
	searchbox.entry:SetGhostText("Search by the job name...")
	...
	searchbox.city.PaintOver = function(self, w, h) ui.DrawText(self.name, "DermaDefaultBold", w / 2, h / 2, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) end
	...
	searchbox.btn:SetText("Search")
	searchbox.btn:SetTextColor(color_white)
	searchbox.btn:SetFont("DermaDefaultBold")
	...
	joblayout.fill = function(self, filter)
		...
		for _, cat in SortedPairsByMemberValue(categories, "sortOrder") do
			...
			for _, job in SortedPairsByMemberValue(cat.members or {}, "sortOrder") do
				...
				local desc = job.description and job.description:gsub("[ ]+", " ") or "No description given."
				jp:SetTooltip(" " .. desc)
